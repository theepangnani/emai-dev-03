...
- [`docs/design/CB-CMCP-001-batch-implementation-plan.md`](../docs/design/CB-CMCP-001-batch-implementation-plan.md) (DDD model, M0–M5 batches)
- [`docs/design/CB-CMCP-001-cost-model.md`](../docs/design/CB-CMCP-001-cost-model.md) (per-content-type token cost, 3 volume tiers, $0.12/artifact spend cap)

**Status:** **M0 engineering complete on `integrate/cb-cmcp-001-m0`** as of 2026-04-28. 13 stripes shipped via parallel autonomous worktree agents (Wave 1: 6, Wave 2: 5, Wave 1.5: 1, Wave 2.5: 1) with 2× `/pr-review` per stripe + 30-issue audit trail. **Integration → master merge pending user approval.** Eleven strategic decisions all locked (#4361–#4371; user accept-all 2026-04-27). Four authenticity amendments (A1–A4) bound as requirements per §6.150.1.

#### M0 stripes shipped (master sequence after integration → master merge)

| Stripe | Wave | Issue | PR | SHA | Scope |
|---|---|---|---|---|---|
| 0D-1 | 1 | #4417 | #4418 | `8de33839` | Cost model doc (D8=B; $0.12/artifact cap, 3 volume tiers) |
| 0B-2 | 1 | #4416 | #4419 | `fd709e8d` | Two-pass extractor CLI (D5=B AI-side check) |
| 0A-2 | 1 | #4413 | #4420 | `e388c985` | Extend `study_guides` per D2=B (8 new columns + `SELF_STUDY` state for D3=C) |
| 0B-1 | 1 | #4415 | #4421 | `7b6f8a94` | Port phase-2 curriculum REST API + `cmcp.enabled` feature flag |
| 0A-3 | 1 | #4414 | #4424 | `57bee2b3` | Add `BOARD_ADMIN` + `CURRICULUM_ADMIN` roles |
| 0A-1 | 1 | #4412 | #4425 | `7a773582` | CEG schema DDL (4 tables: `ceg_subjects`, `ceg_strands`, `ceg_expectations`, `curriculum_versions`) + pgvector/SQLite fallback |
| 0B-3a | 2 | #4428 | #4432 | `9fd5bc3d` | Curriculum-admin review backend (4 endpoints under `/api/ceg/admin/review/*`) |
| 0A-4 | 2 | #4427 | #4433 | `b1e87310` | Migration plumbing audit + 22 tests + reference comment block |
| 0B-4 | 2 | #4430 | #4436 | `54f51f23` | Embedding backfill CLI (`text-embedding-3-small` via dev-03 abstraction) |
| 0B-3b | 2 | #4429 | #4439 | `b3d1afc0` | Curriculum-admin review frontend (`/admin/ceg/review` page, role-gated) |
| #4426 | 2 | #4426 | #4440 | `e8f02d6a` | Bridge 0B-1 routes to new normalized CEG schema (removes 503 fallback) |
| Plan→cost-model link | 1.5 | — | #4444 | `2b128769` | Hyperlink plan §10 cost-overrun row to cost-model doc |
| Wave 2.5 cleanup | 2.5 | #4441 #4442 #4443 | #4445 | `b12cdd29` | Curriculum routes: filter `active=True` + `review_state='accepted'`; case-insensitive subject lookup; drop redundant `q.lower()` before ILIKE |

**Net diff vs master:** 30 files changed, 9,178 insertions, 2 deletions. **172 passed, 1 skipped** on full CMCP test suite at integration HEAD `b12cdd29`. Master snapshot tag `pre-cb-cmcp-001-m0` at `088a317e` (rollback-ready).

#### M0 deliverables NOT in this batch (deferred)

- **0B-5 Phase-1 seed run** — extracts real Gr 1–8 Math/Lang/Sci/SocStudies from Ontario Ministry PDFs. Gated on D5=B OCT-certified curriculum reviewer onboarding (founder task).
- **0C-1 OCT reviewer hire** — process item; ~$6–8k CAD; ~80h Phase 1.
- **0C-2 Accuracy audit framework** — `cli/audit_ceg.py` + ≥99% Ministry-code accuracy gate.
- **0C-3 Audit publishing** — process item; CEG accuracy report alongside `ceg_versions.released_at`.

These are M0-process completions, not engineering work. M0 ships to master with engineering-complete state; OCT reviewer onboarding gates the next milestone (real CEG seed → M1 generation pipeline).
