...
def require_feature(flag_key: str):
    """Dependency factory that checks a feature flag is enabled for the current user.

    Usage:
        @router.get("/foo")
        def foo(_flag=Depends(require_feature("grade_tracking")), ...):
            ...

    Raises 403 if the flag is disabled for the requesting user.
    """
    def checker(
