...
async def upload_report_cards(
    ...
    uploaded: list[UploadedReportCard] = []
    failures: list[dict] = []
    ...
    for f in files:
        ...
        uploaded.append(UploadedReportCard(
            id=rc.id,
            filename=filename,
            file_size=len(content),
            text_extracted=bool(text_content),
            school_name=rc.school_name or "",
            grade_level=rc.grade_level or "",
            term=rc.term or "",
        ))
...
@router.get("/{student_id}", response_model=list[ReportCardItem])
...
async def career_path_analysis(
    ...
    if existing:
        ...
        return CareerPathResponse(
            student_id=student_id,
            content=content,
            report_cards_used=len(cards_with_text),
            created_at=existing.created_at.isoformat() if existing.created_at else "",
        )
