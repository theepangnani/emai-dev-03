...
def check_ai_usage(user: User, db: Session, cost: int = 1) -> None:
    ...
    Args:
        user: The user to check credits for
        db: Database session
        cost: Number of credits this action will consume (default 1).
              Pass 0 for free actions (always passes).
    ...
    if wallet:
        ...
        if total >= cost:
            return  # Wallet has enough for this action
