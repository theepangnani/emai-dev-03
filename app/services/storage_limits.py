...
def check_upload_allowed(user: User, file_size: int) -> None:
    ...
    if file_size > upload_limit:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail=f"File size ({_fmt(file_size)}) exceeds per-file limit ({_fmt(upload_limit)}).")
    if storage_used + file_size > storage_limit:
        remaining = max(0, storage_limit - storage_used)
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail=f"Upload would exceed storage quota. Used: {_fmt(storage_used)}, Limit: {_fmt(storage_limit)}, Remaining: {_fmt(remaining)}.")
...
def get_storage_info(user: User) -> dict:
    ...
    return {"storage_used_bytes": used, "storage_limit_bytes": limit, "upload_limit_bytes": upload_limit, "storage_used_pct": round(pct, 1), "warning": pct >= 80, "critical": pct >= 90}
