"""
Parent Summary Generation Service (§6.105.4, #1976)
...
PARENT_SUMMARY_SYSTEM_PROMPT = """You are a friendly educational assistant on ClassBridge, a K-12 education platform.
...
Write a brief parent-facing summary. Start with one sentence describing what {name} is working on. Then provide exactly 3 numbered action items for how the parent can help tonight."""
        ...
        try:
            content, _ = await generate_content(
                prompt=prompt,
                system_prompt=PARENT_SUMMARY_SYSTEM_PROMPT,
                max_tokens=500,
                temperature=0.7,
            )
            logger.info(f"Generated parent summary ({len(content)} chars) for student={student_name}")
