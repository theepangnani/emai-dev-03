...
The dispatcher passes ``kid_id`` (the child's User row id, or
``None`` when the surfaced artifact is parent-self-generated). This is
used to label the digest block — when ``kid_id`` is supplied **and**
matches the artifact's ``user_id``, the block's ``kid_name`` is the
linked child's first name; otherwise (parent-self-generated case)
``kid_name`` is the parent's first name. The renderer never invents a
name from the artifact alone.
...
def _derive_one_line_description(artifact: Any) -> str:
    ...
    for raw_line in content.splitlines():
        ...
        if len(line) > _DESCRIPTION_MAX_CHARS:
            return line[: _DESCRIPTION_MAX_CHARS - 1].rstrip() + "…"
        return line
...
def _resolve_kid_name(
    ...
    *,
    ...
    owned by the parent themselves). The dispatcher can render the
    block without a kid label in that case.
    """
    # Lazy imports — match the rest of the CMCP service layer (avoids
    ...
    # Validate that the artifact's user_id belongs to one of the
    # parent's linked children. If kid_id was provided, also require it
    # to match — guards against the dispatcher passing a kid_id that
    # doesn't actually own the artifact (defensive — the dispatcher
    # owns this contract but we don't trust unchecked input).
    child_student_ids = [
        row[0]
    ...
    if artifact_user_id not in matching_user_ids:
        return None
    if kid_id is not None and kid_id != artifact_user_id:
        # Dispatcher passed a kid_id that doesn't match — return None
        # for the name (the visibility check downstream will still
        # gate the actual block render).
        return None
...
def _parent_can_see_artifact(
    ...
    *,
    ...
    pulling in role-resolution helpers — the renderer is invoked with a
    parent id, not a User row, so role checks are owned by the
    dispatcher.
    """
    ...
    # Resolve the parent's linked children once — cheap on a parent
    # with the typical 1-3 kids, and we use the result for both the
    # user-id and the course-enrollment branch.
    child_student_ids = [
        row[0]
        for row in db.query(parent_students.c.student_id)
        .filter(parent_students.c.parent_id == parent_id)
    ...
    if getattr(artifact, "user_id", None) in child_user_ids:
        return True
...
def render_cb_cmcp_artifact_summary(
    ...
    if not _parent_can_see_artifact(
        db, parent_id=parent_id, artifact=artifact
    ):
        logger.info(
    ...
    kid_name = _resolve_kid_name(
        db, parent_id=parent_id, kid_id=kid_id, artifact=artifact
    )
