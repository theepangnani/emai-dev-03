...
def waitlist_entry(db_session):
    ...
    existing = db_session.query(Waitlist).filter(Waitlist.email == email).first()
    if existing:
...
def expired_entry(db_session):
    ...
    if existing:
        existing.status = "approved"
        existing.invite_token = token
        existing.invite_token_expires_at = datetime.now(timezone.utc) - timedelta(days=1)
        db_session.commit()
        db_session.refresh(existing)
        return existing
...
class TestJoinWaitlist:
    def test_join_waitlist_success(self, client, db_session):
        from app.models.waitlist import Waitlist
        db_session.query(Waitlist).filter(Waitlist.email == "wl_alice@test.com").delete()
        db_session.commit()
    ...
    def test_join_waitlist_duplicate_email(self, client, db_session):
        from app.models.waitlist import Waitlist
        email = "wl_dup@test.com"
        db_session.query(Waitlist).filter(Waitlist.email == email).delete()
        db_session.commit()

        payload = {"name": "Dup User", "email": email, "roles": ["parent"]}
...
class TestVerifyToken:
    def test_verify_token_valid(self, client, waitlist_entry):
        resp = client.get(f"/api/waitlist/verify/{waitlist_entry.invite_token}")
    ...
    def test_verify_token_expired(self, client, expired_entry):
        resp = client.get(f"/api/waitlist/verify/{expired_entry.invite_token}")
...
class TestAdminWaitlist:
    ...
    def test_admin_approve(self, client, users, db_session):
        ...
        if existing:
            ...
            entry = Waitlist(name="To Approve", email=email, roles=["student"], status="pending")
            db_session.add(entry)
            db_session.commit()
            db_session.refresh(entry)

        headers = _auth(client, users["admin"].email)
        resp = client.patch(f"/api/admin/waitlist/{entry.id}/approve", headers=headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_admin_decline(self, client, users, db_session):
        ...
        if existing:
            ...
            entry = Waitlist(name="To Decline", email=email, roles=["parent"], status="pending")
            db_session.add(entry)
            db_session.commit()
            db_session.refresh(entry)

        headers = _auth(client, users["admin"].email)
        resp = client.patch(f"/api/admin/waitlist/{entry.id}/decline", headers=headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_admin_remind(self, client, users, db_session):
        ...
        if existing:
            ...
            entry = Waitlist(
                name="Remind Me", email=email, roles=["parent"], status="approved",
