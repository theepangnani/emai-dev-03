...
def test_stream_worksheet_mode_passes_3000_max_tokens(client, db_session):
    """`mode='worksheet'` must request ``max_tokens=3000`` (shared with full) (#4382)."""
    make_user(db_session, email="tutor_mode_worksheet_tokens@test.com")
    set_tutor_flag(db_session, enabled=True)
    ...
    try:
        with patch(
            ...
            headers = _auth(client, "tutor_mode_worksheet_tokens@test.com")
            resp = client.post(
                "/api/tutor/chat/stream",
                json={"message": "practice problems please", "mode": "worksheet"},
                headers=headers,
            )
            assert resp.status_code == 200
            _ = resp.text  # drain the stream so the route runs to completion
