...
def test_stream_request_accepts_mode_worksheet(client, db_session):
    """Request body validation accepts ``{"mode": "worksheet"}`` (#4382)."""
    make_user(db_session, email="tutor_mode_worksheet@test.com")
    set_tutor_flag(db_session, enabled=True)
    ...
    try:
        with patch(
            ...
            headers = _auth(client, "tutor_mode_worksheet@test.com")
            resp = client.post(
                "/api/tutor/chat/stream",
                json={"message": "give me practice", "mode": "worksheet"},
                headers=headers,
            )
        assert resp.status_code == 200
    finally:
