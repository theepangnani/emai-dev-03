...
class TestMetadataExtraction:
    ...
    def test_extract_secondary_metadata(self):
        ...
        # Secondary interim format uses "School Name:" pattern — may or may not match
        # At minimum, verify the function returns a dict without errors
        assert isinstance(result, dict)
        # student_name may be extracted
        if result.get("student_name"):
            assert "Thanushan" in result["student_name"] or "Kokulatheepan" in result["student_name"]
...
class TestUploadEndpoint:
    def test_upload_single_pdf(self, client, db_session, src_users):
        ...
        # Upload endpoint should accept the file (200) or handle gracefully
