...
import type { QuickAction } from './RoleQuickActions';
...
export function DashboardLayout({ children, welcomeSubtitle, sidebarActions, headerSlot }: DashboardLayoutProps) {
  ...
  const parentQuickActions = useMemo<QuickAction[]>(() => {
    ...
    return [
      ...
      { icon: QA_ICON_HELP, label: 'Help My Kid', onClick: () => navigate('/my-kids') },
      { icon: QA_ICON_QUIZ, label: 'Quiz History', onClick: () => navigate('/quiz-history') },
      { icon: QA_ICON_ADD_CHILD, label: 'Add Child', onClick: () => navigate('/my-kids') },
  ...
  const navItems = useMemo(() => {
    ...
    if (user?.role === 'admin') {
      ...
      items.push(
        ...
        { label: 'Customer DB', path: '/admin/contacts', group: 'Admin Tools' },
      );
    }
    ...
    <>
      ...
      <div className="dashboard-body" aria-hidden={menuOpen || undefined}>
        ...
        <main id="main-content" className="dashboard-main-full" tabIndex={-1}>
          ...
          {parentQuickActions.length > 0 && location.pathname !== '/dashboard' && (
            <RoleQuickActions actions={parentQuickActions} maxVisible={3} />
          )}
