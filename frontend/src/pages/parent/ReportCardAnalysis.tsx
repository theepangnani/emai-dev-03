...
import type { SchoolReportCard, FullAnalysis, CareerPathAnalysis, GradeAnalysisItem, ImprovementArea, ParentTip, GradeTrend, CareerSuggestion } from '../../api/schoolReportCards';
...
export function ReportCardAnalysis() {
  ...
  const handleAnalyze = useCallback(async (card: SchoolReportCard) => {
    ...
    try {
      ...
      setReportCards(prev =>
        prev.map((c: SchoolReportCard) => c.id === card.id ? { ...c, has_analysis: true } : c)
      );
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to analyze report card.');
    } finally {
  ...
  const handleCareerPath = useCallback(async () => {
    ...
    try {
      ...
      setCareerError(err.response?.data?.detail || 'Failed to generate career path analysis.');
    } finally {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      setUploadError(err.response?.data?.detail || 'Failed to upload report card.');
    } finally {
      ...
      <div className="rca-container">
                      ...
                      <div className="rca-card-actions">
                        ...
                        {card.has_analysis ? (
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); handleViewAnalysis(card); }}
                          ...
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); handleAnalyze(card); }}
                            disabled={analyzeLoadingId === card.id}
                          >
                    ...
                    {expandedCardId === card.id && selectedAnalysis && (
                      <div className="rca-card-body">
                        ...
                        <div className="rca-analysis-section">
                          ...
                          {expandedSections.subjects && selectedAnalysis.grade_analysis.map((subj: GradeAnalysisItem, i: number) => (
                            <div key={i} className="rca-subject-card">
                              ...
                              {subj.feedback && <p>{subj.feedback}</p>}
                            </div>
                          ))}
                        </div>

                        {/* Learning Skills */}
                        {selectedAnalysis.learning_skills && selectedAnalysis.learning_skills.ratings.length > 0 && (
                          <div className="rca-analysis-section">
                            <h4 onClick={() => toggleSection('strengths')}>
                              <span className="rca-toggle">{expandedSections.strengths ? '\u25B2' : '\u25BC'}</span>
                              Learning Skills ({selectedAnalysis.learning_skills.ratings.length})
                            </h4>
                            {expandedSections.strengths && (
                              <ul className="rca-rec-list">
                                {selectedAnalysis.learning_skills.ratings.map((r: { skill: string; rating: string }, i: number) => (
                                  <li key={i}>
                                    <span className="rca-rec-icon">{'\u2705'}</span>
                                    <strong>{r.skill}</strong>: {r.rating}
                                  </li>
                                ))}
                              </ul>
                            )}
                        ...
                        {selectedAnalysis.parent_tips.length > 0 && (
                          <div className="rca-analysis-section">
                            <h4 onClick={() => toggleSection('recommendations')}>
                              <span className="rca-toggle">{expandedSections.recommendations ? '\u25B2' : '\u25BC'}</span>
        ...
        {showUploadModal && (
          <div className="rca-modal-overlay" onClick={() => setShowUploadModal(false)}>
            <div className="rca-modal" onClick={(e) => e.stopPropagation()}>
